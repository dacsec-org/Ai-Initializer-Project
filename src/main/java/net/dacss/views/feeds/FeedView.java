...
/**
 * this will be refactored into the view for all USER-AI conversations and
 * the crud operations for the user(frontend).
 */
@PageTitle("Feed")
@Route(value = "feed", layout = MainView.class)
@Menu(order = 3, icon = LineAwesomeIconUrl.LIST_SOLID)
public class FeedView extends Div implements AfterNavigationObserver {
    ...
    private HorizontalLayout createCard(Person person) {
        ...
        /* save this button to upvote returns!! */
        Icon upVote = VaadinIcon.THUMBS_UP.create();
        upVote.addClassName("icon");
        ...
        actions.add(upVote, likes, commentIcon, comments, shareIcon, shares);
    ...
    private static Person createPerson(String image, String name, String date) {
        Person p = new Person();
        p.setImage(image);
        p.setName(name);
        p.setDate(date);
        p.setPost("In publishing and graphic design, Lorem ipsum is a placeholder text commonly used to demonstrate the visual form of a document without relying on meaningful content (also called greeking).");
        p.setLikes("1K");
        p.setComments("500");
        p.setShares("20");
